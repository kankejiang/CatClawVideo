...
// ⚠️ 逐字取自 TVBox 参考仓库 app/src/main/java/com/p2p/P2PClass.java（AGPL-3.0）。
//    libp2p.so 通过【静态 JNI 符号】绑定到本类：Java_com_p2p_P2PClass_doxstarthttpd 等，
//    且 .so 内无 JNI_OnLoad（非动态注册）——因此 **包名/类名/方法签名一个字都不能改**，
//    改了就 UnsatisfiedLinkError（找不到 native 实现）。
//    本类用途：荐片（csp_Jianpian）播放依赖宿主提供的本地 P2P HTTP 服务。
