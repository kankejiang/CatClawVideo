...
## 11. 排障实录：同一 IP 跑两个 BT 客户端互相挤兑（2026-09-10）

**症状**：同一个磁力，Motrix 6.2MB/s（35 做种/102 节点，还在越来越快），猫爪整包下载
只有 10KB/s（2 做种/2 节点）。前面的第 6~10 节都修过了，仍然复现。

**日志现场**（`bt.log`，任务启动后的演化）：

```
21:41:42  候选=68  实连=1  速度=2.1KB/s     ← tracker announce 正常，拿到了 68 个候选
21:41:52  候选=84  实连=3                     ← 候选涨到 84
21:42:02  候选=20                              ← 5 秒内候选从 82 掉到 20（连接全部失败被丢弃）
21:42:12  候选=0   实连=2  已下=163840 卡死    ← 84 个候选全军覆没，之后 4 分钟再无补充
```

**关键反差**：tracker 状态全是 `[Ok]`、metadata 1.4s 就绪——说明 tracker 链路没问题；
84 个候选出站连接几乎全部失败，这才是异常点。

**对照实验（决定性）**：Motrix 的任务一暂停，**10~30 秒内**猫爪的连接数从 2-3 涨到
58-61，速度冲到 8-12MB/s，候选回升到 76-98。反之恢复 Motrix 后 5 秒内它就握上 43 条
连接、30 秒爬到 1.9MB/s（这就是"Motrix 越来越快"的冷启动曲线）。

**根因**：两个 BT 客户端在同一台机器、同一个公网 IP 上同时下载同一种子，互相挤占：

1. **入站被废**——Motrix 的 aria2c 占着 `bt-settings.json` 里配置的 21301/26701，
   猫爪每次启动都回退随机端口（日志每次都有"已被占用，改用随机端口"）。随机端口
   意味着 tracker 上报的地址不可达、路由器无映射 → **一条入站连接都收不到**。
2. **出站被拒**——swarm 里大部分 peer 同时被 Motrix 连着（43-67 条活跃连接）。
   多数客户端/做种端对同一来源 IP 的连接数有限制，我们的出站连接被成批拒绝，
   于是候选池被"尝试失败"快速耗干。
3. Motrix 暂停 → 它的连接释放 → 我们同样的出站连接立刻全部成功。

**次要因素**（同样真实，但非主因）：
- **MonoTorrent 3.0.1 没有 uTP**（11 个程序集里搜不到任何 Utp 痕迹，纯 TCP）；
  aria2 走 uTP(UDP)+TCP，在 NAT 密集的国内 swarm 里可触达 peer 明显更多。
- tracker 覆盖面：猫爪 14 个（IP 型、单 tier）vs Motrix 40 个（含域名型）。
- DHT 端口每次随机 + 节点缓存仅 10 个种子节点 → DHT 贡献低。
- 实测向这批 tracker 发 BEP15 announce 被静默忽略（connect 正常）→ 存在按 IP 限流。

**结论 / 对策**：
1. **同一时间只跑一个 BT 客户端**——最立竿见影。下载时退出 Motrix（或反之）。
2. 端口冲突的处理不该是"每次随机"：回退后应**持久化新端口**，否则 tracker 永远
   拿到一个不可达端口。更好的做法是端口被占时直接改默认值并写回配置。
3. tracker 列表扩充（30-40 个，含域名型），必要时分 tier。
4. 追平 aria2 的连接能力需要 uTP，属引擎级限制；短期可考虑设置里提供
   "调用系统 aria2 下载"的选项。
